...
export const handler: Handler = async (event: HandlerEvent) => {
  ...
  const body: APIGenerateRequest = JSON.parse(event.body || '{}');
  ...
  if (body.diagramType) {
    outputType = 'diagram';
  } else if ((body as any).mode === 'diagram') {
    outputType = 'diagram';
  ...
  } else if ((body as any).mode === 'document') {
    outputType = 'document';
  } else {
    const classification = await classifyPrompt(body.prompt);
  ...
  try {
    ...
    return {
      statusCode: 500,
      headers: corsHeaders,
      body: JSON.stringify(createErrorResponse(code, err.message ?? 'Generation failed')),
    };
  }
};
